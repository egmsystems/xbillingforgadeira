...
/**
 *
 * xBilling Module for ZPanel 10.0.2
 * Version : 100
...
class module_controller {
    ...
    static function getModuleNav() {
       ...
       $nav_html .='&nbsp;&nbsp;<strong>|</strong>&nbsp;&nbsp;';
        ...
        if(self::$view != 'new_account'){
            $nav_html .='<a href="javascript:_view(\'new_account\');">New Account</a>';
        } else {
            $nav_html .= '<strong>New Account</strong>';
        }
        /*$nav_html .='&nbsp;&nbsp;<strong>|</strong>&nbsp;&nbsp;';
        if(self::$view != 'help'){
            $nav_html .='<a href="javascript:_view(\'help\');">Help</a>';
        } else {
            $nav_html .= '<strong>Help</strong>';
        }*/
        return $nav_html;
    }
    ...
    static function doUpdateSettings(){
        ...
        $setting_keys = array('company_name','email_address','company_logo_path','currency',
                               ...
                               'invoice_reminder_message','pending_invoice_delete_days',
                               'renewal_reminder_days','renewal_reminder_message',
                               'welcome_message','renewal_message',
    ...
    static function doDeletePeriod() {
        ...
        if((int) $formvars['period_id']){
            if (self::ExecuteDeletePeriod((int) $formvars['period_id'])) {
                self::$ok = true;
                return true;
            }        
        } else {
       ...
       if(!isset($urlvars['pid'])){
        $ret_html = 'An error occured while fetching package service periods.';
         ...
         if (is_array($package_periods) && count($package_periods) > 0){
            ...
            $ret_html .= '<tr><td height="20" align="left" width="50%"><strong>Service Period</strong></td>';
            $ret_html .= '<td align="left" width="50%"><strong>Amount</strong></td></tr>';
            ...
            foreach($package_periods as $row_p) {
                ...
                $ret_html .= '<tr><td height="20" align="left">'.$period_name.'</td>';
                $ret_html .= '<td align="left"><input name="period_amount['.$row_p['package_period_id'].']" type="text" id="period_amount_'.$row_p['period_id'].'" style="border: 0px; width: 40px;" value="'.$period_amt.'"" />'.self::getSettingCurrency().'</td></tr>';
            }
            ...
            $ret_html = '<br />
                            There are no service periods to display, please add service periods first.';
         }        
    ...
    static function doUpdatePackagePeriods(){
        ...
        if(is_array($formvars['period_amount'])){
            ...
            echo('<div style="color: #4e9a06;">'.ui_language::translate("Package prices updated successfully.").'</div>');
            exit;        
        }
    }  
    /* Hosting Packages */
    ...
    static function getPaymentOptions(){
        ...
        if (!fs_director::CheckForEmptyValue($methods)) {
            ...
            if(self::getisAddMethod() && !count($res)){
                array_push($res, array('name' => 'There are no additional payment option to add',
        ...
        if(is_array($fields)){
            foreach($fields as $field){
                ...
                $html .= '<tr id="payment_field_'.$x.'">
                                ...
                                <td><input name="field_labels[]" type="text" value="'.$field['label'].'" /></td>
                                <th nowrap="nowrap">'.ui_language::translate("Field Name").':</th>
                                <td><input name="field_names[]" type="text" value="'.$field['name'].'" /></td>
    ...
    static function doDeletePaymentOption() {
        ...
        if((int) $formvars['option_id']){
            if (self::ExecuteDeletePaymentOption((int) $formvars['option_id'])) {
                self::$ok = true;
                return true;
            }        
        } else {
        ...
        if($payment_option_id){
            ...
            if (!fs_director::CheckForEmptyValue($fields)) {
                while ($row = $fields->fetch()) {
                   ...
                   //array_push($res, array('label' => $row['field_label'],'name' => $row['field_name']));  
                   $html .= '<tr><th nowrap="nowrap">'.$row['field_label'].'</th>
                                            <td><input name="'.$row['field_name'].'" type="text" id="'.$row['field_name'].'" value="'.$field_info['field_value'].'" style="border: 0px;" /></td>
                                        </tr>';
                }
            }            
        ...
        if (!fs_director::CheckForEmptyValue($orders)) {
            while ($row = $orders->fetch()) {
              ...
              $order_status = ($row['invoice_status'] == 1) ? 'Paid':'Pending';
                ...
                //view
                $button_html = '<button class="fg-button ui-state-default ui-corner-all" type="submit" id="button" name="inView_'.$row['invoice_id'].'" id="inView_'.$row['invoice_id'].'" value="inView_'.$row['invoice_id'].'" onclick="_change_action(\'frmOrders\',\'ViewOrder\', \''.$row['invoice_id'].'\');">'.ui_language::translate("View").'</button>&nbsp;';

              if(!$row['invoice_status']){
                //edit
                $button_html .= '<button class="fg-button ui-state-default ui-corner-all" type="submit" id="button" name="inEdit_'.$row['invoice_id'].'" id="inEdit_'.$row['invoice_id'].'" value="inEdit_'.$row['invoice_id'].'" onclick="_change_action(\'frmOrders\',\'EditOrder\', \''.$row['invoice_id'].'\');">'.ui_language::translate("Edit").'</button>';
              }
    ...
    static function getOrderInfoAmount(){
        global $controller;
        $formvars = $controller->GetAllControllerRequests('FORM');
        
        if(isset($formvars['order_id'])){
            $order_amount = self::getOrderInfo($formvars['order_id'], 'invoice_total_amount',self::$module_db.'.x_invoices');
...
?>';
             ...
             while ($row = $res->fetch()){
                ...
                if($today_date >= $remind_date){
                    ...
                    if($invoice_reminder_message && $user_info['email']){
                        ...
                        $phpmailer = new sys_email();
                        $phpmailer->Subject = "Your Invoice #".$row['invoice_reference'].' at '.$company_name;
                ...
                if($row['vh_expiry_ts']){
                    ...
                    if($today_date >= $remind_date){
                        ...
                        if($renewal_reminder_message && $user_info['email']){
                            ...
                            $phpmailer = new sys_email();
                            $phpmailer->Subject = "Your Invoice #".$invoice_reference.' at '.$company_name;
    ...
    static function CreateRenewalInvoice($domain_id){
        ...
        //fetch domain info
          ...
          //fetch previous invoice/order information
    ...
    static function ExecuteUpdateOrder($order_id, $completed_yn){
        ...
        if($order_id && $completed_yn){
           ...
           //find and update orders relating to this invoice
            ...
            //order info
            if(is_array($order_info)){
                $orders['order_id'] = $order_info['order_id'];
                $sql = $zdbh->prepare("SELECT order_vh_fk,package_period_id_fk FROM 
                                            ".self::$module_db.".x_orders 
                                            WHERE order_id=:id;");
                $sql->bindParam(':id', $order_info['order_id']);
                $sql->execute();
                $order_info = $sql->fetch();
                if(is_array($order_info)){
                    $orders['order_vh_fk'] = $order_info['order_vh_fk'];
                    $orders['package_period_id_fk'] = $order_info['package_period_id_fk'];
                } else {
                    $orders = '';
                }
            }
