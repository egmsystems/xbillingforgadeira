...
class webservice extends ws_xmws {
	...
	// Se incluyen cosntantes requeridas para la funcion que creará las tiendas
	const HTDOCS = "/var/zpanel/hostdata/";
	const BASIC_PLAN = "/home/opencart-1.5.6/upload";
	const PROFESIONAL_PLAN = "/home/opencart-1.5.6/upload";
	const PREMIUM_PLAN = "/home/opencart-1.5.6/upload";
	...
	/**
	 * This method installes and configures the opencart for the client´s plan.
	 * Version : 100
	 * @author Diego Rubio Abujas
	 * Email : dbiosag@gmail.com
 	 * @desc Creates foolder and database for opencart. Installs and clean the installer´s folder.
	 */
	public function CreateOCShop($userid,$password,$email,$plan){
		switch ($plan) {
			case '3': // Profesional
				$planfolder = PROFESIONAL_PLAN;
				break;
			case '4': // Premium
				$planfolder = PREMIUM_PLAN;				
				break;	
			case '5': // Basico
				$planfolder = BASIC_PLAN;
				break;
			default:
				exit();
		}
		...
		// 1º Create the destiny's foolder in the htdocs's client.
		if(!mkdir($destinyfoolder,0755)){
			die('The folder of the opencart cannot be created.');
		}
		// ... 			
        ...
        if((isset($request_data['xmws']['content']['zpx_uid']) && $request_data['xmws']['content']['zpx_uid'] > 0) && (isset($request_data['xmws']['content']['ref'])) && (isset($request_data['xmws']['content']['transaction_id'])) && (isset($request_data['xmws']['content']['date']))){
            ...
            if(!$payment){
                ...
                $payment_result['message'] = $payment;
 			    ...
 			    $user = ctrl_users::GetUserDetail($zpx_uid);	 
				CreateOCShop($zpx_uid['userid'],$zpx_uid['password'],$zpx_uid['email'],$zpx_uid['packageid']);              
